...
func getClient(service string, number_loop int) {
	...
	if err == nil {
		if resp.StatusCode() == 200 {
			fmt.Println(number_loop, "status code", resp.StatusCode(), resp.Header(), "on service ", service)
		} else {
			color.Red(a + " error endpoint with status code : " + b + " on service " + service)
			Mail(service, resp.StatusCode(), "", string(resp.Body()))
		}
	} else {
		color.Red(a + " endpoint failur : " + err.Error())
		Mail(service, resp.StatusCode(), err.Error(), string(resp.Body()))
	}
}
...
func mainProgram() {
	for true {
		...
		file, err := os.Open("/etc/monitorendpoint/nameapp.conf")

		if err != nil {
			log.Fatalf("failed opening file: nameapp.conf at /etc/monitorendpoint directory")
		}
		...
		for num_app, eachline_name_of_app := range txtlines {
			...
			//running goroutine/
			//go getClient(eachline_name_of_app, num_app)
...
func Mail(s string, status_code int, resp string, body_text string) {
	var conf Config
